...
/**
 * MasterData: enthält Schuljahre und Schulen
 */
...
for (const student of sampleStudents) {
  ...
  for (let j = 0; j < numEntries; j++) {
    ...
    const erfolgRating = ratingRand < 0.6 ? 'positiv' : (ratingRand < 0.85 ? 'negativ' : '');

    sampleEntries.push({
      id: entryIdCounter++,
      studentId: student.id,
      date: getRandomDate(new Date(2024, 7, 1), new Date()),
